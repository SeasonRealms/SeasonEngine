...
public static class Database
{
    ...
    private static void CacheClearAll()
    {
        _cache.Clear();
    }
#else
    ...
    private static void CacheClearAll()
    {
        var keys = MemoryCache.Default.Select(kvp => kvp.Key).ToList();
    ...
    public static T GetOne<T>(string name)
    {
        lock (syncLock)
        {
            ...
            if ((dBConfig == null || dBConfig.IsCache) && CacheContains(name))
            {
                t = (T)CacheGet(name);
            }
            else
    ...
    public static List<T> GetList<T>(string name, string tag)
    {
        lock (syncLock)
        {
            ...
            if ((dBConfig == null || dBConfig.IsCache) && CacheContains(dbName))
            {
                list = (List<T>)CacheGet(dbName);
            }
            else
            {
