...
internal class Graphics : IGraphics
{
    ...
    /// <summary>
    /// 2-6 clause 1: mip policy implied by a glTF material channel. The mapping is the same one the three native
    /// backends apply when they call Texture.GetOrCreate, so a channel cannot end up filtered differently here.
    /// </summary>
    static TextureMipPolicy MipPolicyForChannel(string channel) => channel switch
    {
        "normal" => TextureMipPolicy.Normal,
        "metallicRoughness" => TextureMipPolicy.Linear,
        "occlusion" => TextureMipPolicy.Linear,
        _ => TextureMipPolicy.Color,
    };

    /// <summary>Slot-index form of <see cref="MipPolicyForChannel"/>, ordered as in SetTextureNameBySlot.</summary>
    static TextureMipPolicy MipPolicyForSlot(int slot) => slot switch
    {
        1 => TextureMipPolicy.Normal,
        2 => TextureMipPolicy.Linear,
        3 => TextureMipPolicy.Linear,
        _ => TextureMipPolicy.Color,
    };
    ...
    async Task UploadGltfImageTexture(string modelName, string channel, SharpGLTF.Schema2.Image? image, bool deferDecodeToNextFrame = false)
    {
        ...
        {
            result = await _jsRuntime.InvokeAsync<WebTextureUploadResult>("seasonWebGPU.uploadEncodedTexture",
                texName,
                encodedBytes,
                null,
    ...
    async Task<WebTextureUploadResult> UploadGltfImageTextureWithMips(string texName, byte[] encodedBytes, TextureMipPolicy policy)
    {
        try
        {
            // decodeImageBytes is awaited on the JS side (createImageBitmap plus a 2D canvas readback), so it already
            // yields to the browser and needs no separate defer-to-next-frame handling.
            var decoded = await _jsRuntime.InvokeAsync<WebDecodedTexturePixels>("seasonWebGPU.decodeImageBytes", encodedBytes);
            if (decoded == null || decoded.Width <= 0 || decoded.Height <= 0 || decoded.RgbaData == null ||
        ...
        catch (Exception ex)
        {
            DeviceServices.BaseApp.AddLog(LogType.Error,
                $"{DateTime.UtcNow} [ModelLoad] {texName} mip chain upload failed, falling back to direct decode: {ex.Message}");
            return null;
        }
    }

    /// <summary>Shape returned by seasonWebGPU.decodeImageBytes.</summary>
    sealed class WebDecodedTexturePixels
    {
    ...
    void TryReplaceModelTextureWGPU(Model model, TextureUpdateSource source, int slot, Action clearSource)
    {
        ...
        int mipLevelCount = BuildMipChainForUpload(ref rgba, w, h, MipPolicyForSlot(slot));
        _jsRuntime.InvokeVoid("seasonWebGPU.createTextureFromPixels", newName, rgba, w, h, true, mipLevelCount);
        foreach (var prim in allPrims)
        {
    ...
    async Task<string> ResolveSurfaceSlotTexture(string meshName, long meshId, int surfaceIndex, Surface surface, SurfaceTextureSlot slot)
    {
        ...
        if (source.Image != null)
        {
            ...
            // Must precede the call: arguments are evaluated left to right, so folding this into the argument list
            // would pass the pre-chain array alongside a level count that no longer describes it.
            int mipLevelCount = BuildMipChainForUpload(ref rgba, w, h, MipPolicyForSurfaceSlot(slot));
