...
internal class WindowsStoreService : IStoreService
{
    ...
    public async Task<(List<Product>, string)> Query()
    {
        ...
        var results = await WindowsApp.StoreContext.GetAssociatedStoreProductsAsync(filterList);

        if (results.ExtendedError != null)
        {
            if (results.ExtendedError.HResult == IAP_E_UNEXPECTED)
            {
                message = $"Error:{results.ExtendedError.Message}";
            }
            else
            {
                message = $"Error:{results.ExtendedError.Message}";
        ...
        {
            foreach (var result in results.Products.Values)
            {
                var product = new Product()
                {
                    StoreId = result.StoreId,
                    Title = result.Title,
                    Type = result.ProductKind,
                    Price = result.Price.FormattedPrice,
                    InCollection = result.IsInUserCollection
                };
