...
{
    public override Basic.Platform Platform => Basic.Platform.iOS;
    ...
    public override string LoadFilePath(string res)
    {
        ...
        var directory = Path.GetDirectoryName(res)?.Replace('\\', '/');
        var fileName = Path.GetFileNameWithoutExtension(res);
        var ext = Path.GetExtension(res)?.TrimStart('.');

        var bundlePath = NSBundle.MainBundle.PathForResource(fileName, ext, directory);

        if (bundlePath != null && File.Exists(bundlePath))
            return bundlePath;

        var bundleRoot = NSBundle.MainBundle.BundlePath;
        var bundleFile = Path.Combine(bundleRoot, res);
...
internal class iOSAds : IAds
{
    ...
    public string InitAd()
    {
        ...
        catch (Exception ex)
        {
            ...
            DeviceServices.BaseApp.AddLog($"{DateTime.UtcNow} InitAd: {ex}");
        }
        ...
        if (InterstitialAd == null || expired)
        {
            try
            {
                var handler = new InterstitialAdLoadCompletionHandler((ad, err) =>
                {
                    ...
                    DeviceServices.BaseApp.AddLog($"{DateTime.UtcNow} LoadAd: {(ad is null ? mes.NullToString() : ad.ToString())}");
            ...
            {
                DeviceServices.BaseApp.AddLog($"{DateTime.UtcNow} LoadAd: {ex}");
        ...
        try
        {
            ...
            else
            {
                ...
                catch (Exception ex)
                {
                    ...
                    DeviceServices.BaseApp.AddLog($"{DateTime.UtcNow} ShowAd....");
